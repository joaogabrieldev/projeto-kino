...
const Footer = () => {
  ...
  return (
    <motion.footer className="static bottom-0 flex w-full max-w-full bg-[#8B0000] py-3 select-none">
      <div
        className={`${hendrigo.className} flex w-fit flex-col px-4 text-3xl text-gray-300 dark:text-yellow-500`}
      >
        <span className="">João</span>
        <span className="pl-6">Gabriel</span>
      </div>
      <div
        className={`w-full ${onest.className} text-md flex items-center justify-center text-gray-300 dark:text-yellow-500`}
      >
      ...
      <div className="flex flex-row gap-2 px-4 pt-2">
        <span>
          <SiLinkedin
            className={`h-8 w-8 cursor-pointer fill-gray-300 transition-colors duration-200 hover:fill-gray-400 dark:fill-yellow-500 dark:hover:fill-yellow-600`}
